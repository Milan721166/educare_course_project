...
MIDDLEWARE = [
    ...
    # Whitenoise for static files (works only when DEBUG=False)
    'whitenoise.middleware.WhiteNoiseMiddleware',
...
# Local static settings only when DEBUG=True
if DEBUG:
    STATICFILES_DIRS = [
        BASE_DIR / 'static'
    ]

# Production static root (Render uses this)
STATIC_ROOT = BASE_DIR / 'staticfiles'

MEDIA_URL = '/media/'
MEDIA_ROOT = BASE_DIR / 'media'

# Whitenoise static compression in production
if not DEBUG:
    STATICFILES_STORAGE = 'whitenoise.storage.CompressedManifestStaticFilesStorage'
